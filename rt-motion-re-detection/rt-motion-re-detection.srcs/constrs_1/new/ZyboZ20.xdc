...
##Timing Constraints
# TMDS RX recovered pixel clock - declared at 120 MHz (1080p upper bound)
create_clock -name tmds_rx_clk -period 11.000 [get_ports { TMDS_0_clk_p }];

# False paths between async TMDS pixel clock and system clock domains
set_false_path -from [get_clocks tmds_rx_clk] -to [get_clocks sys_clk_pin];
set_false_path -from [get_clocks sys_clk_pin] -to [get_clocks tmds_rx_clk];
